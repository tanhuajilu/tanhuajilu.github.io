...
for i,p in enumerate(ready,1):
    ...
    if state.get(key)==size:
        ...
        if r.status_code==200:
            ...
            fail+=1; print('FAIL',p.name,r.status_code)
    if i%20==0 or i==len(ready):
        print(f'[{i}/{len(ready)}] up={up} skip={skip} fail={fail}', flush=True)
state_file.write_text(json.dumps(state,ensure_ascii=False,indent=2),encoding='utf-8')
print({'book':book,'ready_local':len(ready),'uploaded_now':up,'skipped_state':skip,'failed':fail,'uploaded_total_state':len(state)})
